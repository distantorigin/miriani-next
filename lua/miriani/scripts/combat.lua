...
<triggers>
  ...
  <send>
   ...
   mplay("ship/combat/hit/youHit", "ship", nil, nil, nil, nil, nil, nil, nil, volume_offset)
  ...
  <trigger
   ...
   group="combat"
   script="gagline"
   match="^(?:The room is (?:filled with a .+? light as the laser turrets? (?:are|is) fired|bathed in brilliant .+? light as the ship's (?:laser turrets? )?fires? alternating patterns of festive color)|The ship jerks slightly as its turrets are fired)\.$"
   regexp="y"
   omit_from_output="y"
   send_to="14"
   sequence="100"
  ...
  <trigger
   ...
   group="combat"
   script="gagline"
   match="^You hear a thunk as a chunk of charged weapons-grade bardenium is fired from the ship's cannons\.|A superheated gumdrop is ejected at high velosity from the ship's launchers, leaving a trail of .+ sugar crystals in its wake\.$"
   regexp="y"
   omit_from_output="y"
   send_to="14"
   sequence="100"
