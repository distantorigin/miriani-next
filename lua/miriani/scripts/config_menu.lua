...
function config_menu.show_group(group_name)
  ...
  -- First check if it's an exact match for a known group in group_metadata
  local matched_exact = false
  local metadata = config:get_group_metadata()
  for _, meta in ipairs(metadata) do
    if string.lower(meta.key) == string.lower(group_name) then
      actual_group_key = meta.key
      matched_exact = true
      break
    end
  end
